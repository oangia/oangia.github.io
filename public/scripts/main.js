...
/*const postsConfig = {
  ...
  columns: [
    ...
    },
    {
      render: (post) => {
        ...
        return `<td class="td-description">${escapeHtml(preview)}</td>`;
      }
    },
    {
      render: (post) => {
        ...
        return `
          <td style="text-align: center;">
            ${hasImage ? 
              `<img src="${escapeHtml(post.featuredImage)}" alt="Featured" style="width: 60px; height: 40px; object-fit: cover; border-radius: 4px;">` : 
              '<span style="color: #9ca3af; font-size: 13px;">No image</span>'
            }
          </td>
        `;
      }
  ...
  getFormData: (elements) => ({
    ...
    featuredImage: elements.featuredImageInput.value.trim(),
    publishDate: new Date(elements.publishDateInput.value).getTime()
  }),
